...
#Region "Count SMS"
        Public Function CountSMSmessages(ByVal port As SerialPort) As Integer
            ...
            Try
                ...
                Dim recievedData As String = ExecCommand(port, "AT", 1000, "No phone connected at ")
                ...
                If (recievedData.Length >= 45) AndAlso (recievedData.StartsWith("AT+CPMS?")) Then
                    ...
                    '#Region "Parsing SMS"
                    Dim strSplit As String() = recievedData.Split(",")
...
#Region "Read SMS"
        ...
        Public Function ReadSMS(ByVal port As SerialPort, ByVal p_strCommand As String) As ShortMessageCollection
...
#Region "Send SMS"
        ...
        Public Function sendMsg(ByVal port As SerialPort, ByVal PhoneNo As String, ByVal Message As String) As Boolean
            ...
            Try
                ...
                recievedData = ExecCommand(port, "AT+CMGF=1", 11000, "Failed to set message format.")
                Dim command As [String] = "AT+CMGS=""" & PhoneNo & """"
                recievedData = ExecCommand(port, command, 11000, "Failed to accept phoneNo")
                command = Message & Char.ConvertFromUtf32(26) & vbCr
                recievedData = ExecCommand(port, command, 11000, "Failed to send message")
                '3 seconds
...
#Region "Delete SMS"
        Public Function DeleteMsg(ByVal port As SerialPort, ByVal p_strCommand As String) As Boolean
            Dim isDeleted As Boolean = False
            Try

                '#Region "Execute Command"
                Dim recievedData As String = ExecCommand(port, "AT", 1000, "No phone connected")
                recievedData = ExecCommand(port, "AT+CMGF=1", 1000, "Failed to set message format.")
